...
function getLoginAttempts() {
  if (typeof window === 'undefined') return { count: 0, lastAttempt: null, lockedUntil: null };
  try {
...
export async function authenticateUser(username, password) {
  if (isLockedOut()) {
    return { error: `Too many failed attempts. Try again in ${getLockoutRemaining()} minutes.` };
  }
