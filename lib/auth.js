...
export function getGroupLabel(group) {
  ...
  if (group === 'partner') return 'GP Partner';
  return '';
}
...
// re-sync default user passwords when DEFAULT_USERS change, and
// migrate any legacy plain-text passwords to hashes.
export async function initializeUsers() {
  if (typeof window === 'undefined') return;

  // Pre-compute hashes for all default users (needed for both paths)
  const defaultHashes = {};
  await Promise.all(
    DEFAULT_USERS.map(async (u) => {
      defaultHashes[u.id] = await hashPassword(u.password);
    })
  );
  ...
  if (!stored) {
    // First boot: store hashed default users
    const users = DEFAULT_USERS.map((u) => ({
      ...u,
      password: defaultHashes[u.id],
    }));
    localStorage.setItem(USERS_KEY, JSON.stringify(users));
    return;
  }

  // Existing install: re-sync defaults + migrate plain-text passwords
  try {
    ...
    const migrated = await Promise.all(
      users.map(async (u) => {
        ...
        if (defaultUser && !u.passwordChangedAt) {
          // Default user whose password was never explicitly changed via UI.
          // Always re-sync to the current default hash so that code-level
          // password changes take effect on existing installs.
          const correctHash = defaultHashes[defaultUser.id];
          if (u.password !== correctHash) {
            needsSave = true;
            return { ...u, password: correctHash };
          }
          return u;
        }

        // Non-default user (or explicitly changed) with plain-text — migrate
    ...
    // Add any NEW default users not yet in stored data
    for (const defaultUser of DEFAULT_USERS) {
      if (!migrated.some(u => u.id === defaultUser.id)) {
        needsSave = true;
        migrated.push({
          ...defaultUser,
          password: defaultHashes[defaultUser.id],
        });
      }
    }
